...
export const ItemPage: React.FC<TItemPageProps> = ({ item, isNew = false }) => {
  const [submitted, setSubmitted] = React.useState(false);
  const [isEditing, setIsEditing] = React.useState(isNew);
  const [selectedImageId, setSelectedImageId] = React.useState(
    item?.main_image.split('/').pop() ?? ''
  );
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
      name: item?.name ?? '',
      master_notes: item?.master_notes ?? '',
      description: item?.description ?? '',
    },
  });

  const playersWithItem = players?.filter((player) =>
    item?.Players?.find((p) => p.Player_id === player.id)
  );

  const npcsWithItem = npcs?.filter((npc) =>
    item?.Npcs?.find((n) => n.Npc_id === npc.id)
  );

  const locationsWithItem = locationsInCampaign?.filter((location) =>
    item?.Locations?.find((l) => l.Locations_id === location.id)
  );
